  ...
  try {
    ...
    if (mediaType === "image") {
      ...
      messagePayload = {
        document: source,
        mimetype:
          fileMimetype || getMimeTypeFromExtension(mediaUrl || fileName || ""),
...
messagesRouter.post("/blast", async (c) => {
  ...
  if (!body.deviceId || !body.numbers?.length || !body.message) {
    return c.json(
      { error: "deviceId, numbers, and message are required" },
      400,
    );
  }
...
messagesRouter.delete("/blast/:id", (c) => {
  ...
  if (!stopped)
    return c.json({ error: "Blast not found or already finished" }, 404);
